...
import { getEICodeLedger } from "@/Utils/FetchReports";
import Printer from "@/Utils/Printer";
import moment from "jalali-moment";
import EICodesReportFilter from "../Components/EICodesReportFilter";
...
function ViewEICodesReport() {
    ...
    const LoadRows = (url: string | null | boolean = false) => {
        ...
        if (url)
            ...
            getEICodeLedger(useFilterOptions, (response: any) =>
                callback(response),
            );
    };
    ...
    const onReportPrint = (): void => {
        ...
        getEICodeLedger(
            ...
            function (statements: Array<any>) {
                ...
                <li>
                    <span class="i-title">مصارفو / عوایدو کود</span> :-
                    <span class="i-value"> ${(useFilterOptions.code as any)?.name || ""}  </span>
                </li>
            </div>
            <div>
                <li>
            ...
            `,
                    Header: function () {
                        return `<tr><th>#</th><th>تاریخ</th><th>معاملي ډول</th><th>تفصیل</th><th>مبلغ</th></tr>`;
                    },
                    Data: function () {
                        let _Rows = "";
                        ...
                        statements.forEach((entry: Data, index: number) => {
                            ...
                            _Rows += `<td>${entry.credit_type == "Expense" ? entry.debit_amount : entry.credit_amount} ${entry.currency.symbol}</td>`;
                            _Rows += "</tr>";
                        });
                                ...
                                {stableSort(
                                    ...
                                    <tr key={row.id}>
                                        ...
                                        <td>
                                            ...
                                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                            {(row.credit_type == "bank" ||
                                                row.debit_type == "bank") && (
                                                <Chip color="primary">
                                                    {(row as any).bank?.name}{" "}
                                                    صرافي
                                                </Chip>
                                            )}
                                            {(row.credit_type == "till" ||
                                                row.debit_type == "till") && (
                                                <Chip color="primary">
                                                    {(row as any).till?.name}{" "}
                                                    دخل
                                                </Chip>
                                            )}
                                        </td>
...
export default ViewEICodesReport;
