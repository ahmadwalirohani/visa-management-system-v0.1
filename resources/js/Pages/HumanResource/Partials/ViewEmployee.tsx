...
import { GetEmployees } from "@/Utils/FetchResources";
...
export default function ViewEmployee() {
    ...
    const LoadEmployees = () => {
        ...
        GetEmployees((Employees: Array<object>): void => {
            setRows([...(Employees as Data[])]);
    ...
    const SendEmployeeToForm = (payload: any) => {
        emitEvent("SendPayloadToFormEvent", payload);
    };

    React.useEffect(() => {
        addEventListener(
            "ReloadEmployeesEvent",
            function (isFetch = false): void {
                if (isFetch) LoadEmployees();
            },
        );
        if (!rows.length) LoadEmployees();
    ...
    const onEmployeeStatusChange = (Employee: any): void => {
        ChangeResourceStatus({
            id: Employee.id,
            model: "Employee",
            status: Employee.status as number,
            onSend() {},
            afterChange() {
                setSnackbar({
                    msg: `کارمند په بریالي سره ${Employee.status ? "غیري فعاله" : "فعاله"} سول`,
                    state: "success",
                    is_open: true,
                });
                LoadEmployees();
